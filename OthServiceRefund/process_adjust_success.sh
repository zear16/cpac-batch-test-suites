...
echo -e "\ngo" >> ${SCRIPT}
echo -e "\nset nocount on" >> ${SCRIPT}
echo -e "select I.SO_NBR, INF.ORDER_LINE_NO" >> ${SCRIPT}
echo -e "from PMDB..PM_ADJUST_TRANSACTION AD" >> ${SCRIPT}
echo -e "inner join PMDB..PM_FILE_ADJUST FL on (AD.FILE_ID = FL.FILE_ID)" >> ${SCRIPT}
echo -e "inner join PMDB..PM_ADJUST_TRANSACTION I on (FL.FILE_ID = I.FILE_ID)" >> ${SCRIPT}
echo -e "inner join PMDB..PM_INF_ADJUST_BALANCE_D INF on (I.SO_NBR = INF.SO_NBR and INF.ORDER_LINE_NO = 1)" >> ${SCRIPT}
...
echo -e "left join PMDB..PM_RECEIPT_PREPAID RP on (R.RECEIPT_ID = RP.RECEIPT_ID and R.RECEIPT_DATE = RP.RECEIPT_DATE)" >> ${SCRIPT}
echo -e "left join PMDB..PM_RECEIPT_PAYMENT PM on (R.RECEIPT_ID = PM.RECEIPT_ID and R.RECEIPT_DATE = PM.RECEIPT_DATE)" >> ${SCRIPT}
echo -e "left join PMDB..PM_RECEIPT_DTL_PREPAID RDP on (RD.RECEIPT_DTL_ID = RDP.RECEIPT_DTL_ID and RD.RECEIPT_DATE = RDP.RECEIPT_DATE)" >> ${SCRIPT}
