...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and R.SEQ_NO = 1 and R.FIELD_NAME = 'PRODUCT_ID'" >> ${SCRIPT}
...
echo -e "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,FIELD_NAME=PRODUCT_ID,REMARK=Invalid PRODUCT_ID : ${COUNT}"
...
echo -e "found PM_PREPAID_LOAD_BATCH with BATCH_STATE=C,PROCESS_STATUS=FL,LOAD_TOTAL=1,LOAD_SUCCESS=0,LOAD_ERROR=1"
echo -e "                                 ,TOTAL_AMT=0,SUCCESS_AMT=0,ERROR_AMT=0 : ${COUNT}"
