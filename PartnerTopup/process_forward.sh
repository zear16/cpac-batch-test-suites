...
and LOAD_TOTAL = 3 and LOAD_SUCCESS = 3 and LOAD_ERROR = 0
...
echo "found PM_PREPAID_LOAD_BATCH with BATCH_STATE=L,PROCESS_STATUS=SC,LOAD_TOTAL=3,LOAD_SUCCESS=3,LOAD_ERROR=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
select T.MOBILE_NO
from PM_BATCH_PARTNER_TOPUP T
inner join PM_PREPAID_LOAD_BATCH B on (T.BATCH_ID = B.BATCH_ID)
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=3,4,REMARK='NOT_PREPAID_DATA' : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
#cat ${FORWARD}
#echo "=================================================="
#cat ${WORKING_PATH}/forward_cmp.file
