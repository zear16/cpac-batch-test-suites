...
select @count = count(*)
from PM_INF_BATCH_PARTNER_TOPUP_D
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_INF_BATCH_PARTER_TOPUP_D : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
where B.ORDER_ID = ${ORDER_ID}
and R.SEQ_NO = 1
and R.FIELD_NAME = 'EFFECTIVE DATE'
and R.REMARK = 'Invalid EFFECTIVE DATE'
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1
      ,FIELD_NAME=EFFECTIVE DATE,REMARK=Invalid EFFECTIVE DATE: ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
