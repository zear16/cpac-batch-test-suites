...
select @template_code = 'PE_PARTNER_TOPUP'
, @run_date = '20160616'
--, @file_name = 'AWNOVL_16062016.zip'
, @file_name = 'AWNOVL_16062016001.dat'
...
begin
  ...
  , VERSION_ID, FILE_NAME, ORIGINAL_FILE_NAME, COMPANY_ID
  --, ORIGINAL_FILE_NAME, ZIP_NAME
  , FILE_PATH, ORDER_STATUS)
  values
  ('I', @job_id, @template_code, 'A', @run_date, convert(date, @run_date, 112), convert(date, @run_date, 112)
  , @version_id, @file_name, @file_name, @company_id
  --, 'AWNOVL_*.dat', @file_name
  , '/app/payment/batch/partner_topup', 'W')
