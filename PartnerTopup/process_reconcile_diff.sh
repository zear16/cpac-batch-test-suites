...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_INF_BATCH_PARTNER_TOPUP_D"  >> ${SCRIPT}
echo -e "where ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_INF_BATCH_PARTNER_TOPUP_D : ${COUNT}"

if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi

${EXECUTE_PATH}/call_sproc -c ${CONFIG_NAME} PM_S_TX_LOAD_PM_BATCH_PARTNER_TOPUP_FROM_INF @order_id=${ORDER_ID}
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_BATCH_PARTNER_TOPUP R"  >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_BATCH_PARTNER_TOPUP  : ${COUNT}"
...
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and MT.TABLE_NAME = 'PM_BATCH_PARTNER_TOPUP'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '004'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=004,TABLE_NAME=PM_BATCH_PARTNER_TOPUP : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0810083859'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160616'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = 7782" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '004'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=004,D_MOBILE_NO=0810083859,TRANS_DTM=20160616,FACE_VALUE=7782 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_AMT = 7782" >> ${SCRIPT}
...
echo -e "and R.REF_1_L_REF_2_AMT = 7782" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=004,LAST_BOO=Y,DIFF_BOO=Y"
echo "      ,REF_1_RECORD=1,REF_1_AMT=7782,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=0,REF_2_AMT=0,REF_2_VALIDITY=0"
echo "      ,REF_1_L_REF_2_RECORD=1,REF_1_L_REF_2_AMT=7782,REF_1_L_REF_2_VALIDITY=0"
...
select O.DATA_DATE_FR, T.RECHARGE_CHANNEL, RS.BANK_CODE
from PMDB..PM_RECHARGE T
inner join PMDB..PM_JOB_ORDER O on (T.RECHARGE_DATE = O.DATA_DATE_FR)
inner join PMDB..PM_RECHARGE_SERVICE RS on (T.RECHARGE_CHANNEL = RS.SERVICE_ID)
where RS.BANK_CODE = 603
and O.ORDER_ID = ${ORDER_ID}

select count(*) as COUNT_RECHARGE 
from PMDB..PM_RECHARGE T 
inner join PMDB..PM_JOB_ORDER O on (T.RECHARGE_DATE = O.DATA_DATE_FR) 
inner join PMDB..PM_RECHARGE_SERVICE RS on (T.RECHARGE_CHANNEL = RS.SERVICE_ID) 
where RS.BANK_CODE = 603 
