...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=004,TABLE_NAME=PM_RECHARGE : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
#echo -e "and D.D_MOBILE_NO = '0802001301'" >> ${SCRIPT}
#echo -e "and D.TRANS_DTM = '20160616'" >> ${SCRIPT}
#echo -e "and D.FACE_VALUE = 778200" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '004'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=004 : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_2_AMT = 7782" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=004,LAST_BOO=Y,DIFF_BOO=N"
echo "      ,REF_1_RECORD=1,REF_1_AMT=7782,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=1,REF_2_AMT=7782,REF_2_VALIDITY=0"
