...
where B.ORDER_ID = ${ORDER_ID}
and R.SEQ_NO = 3
and R.FIELD_NAME = 'TOTAL CREDIT TRANSACTION'
and R.REMARK = 'Invalid TOTAL CREDIT TRANSACTION'
...
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=3
      ,FIELD_NAME=TOTAL CREDIT TRANSACTION,REMARK=Invalid TOTAL CREDIT TRANSACTION : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
