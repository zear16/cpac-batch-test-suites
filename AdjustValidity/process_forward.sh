...
and LOAD_TOTAL = 2 and LOAD_SUCCESS = 2 and LOAD_ERROR = 0
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=3,FIELD_NAME='MOBILE_NO'"
echo "      ,REMARK='NOT_PREPAID_DATA' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
