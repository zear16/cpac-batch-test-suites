...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_INF_BATCH_ADJ_VALIDITY_H" >> ${SCRIPT}
echo -e "where ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and ORDER_LINE_NO = 1" >> ${SCRIPT}
echo -e "and ADJUST_DTM = '2016-08-08T16:54:16'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_INF_BATCH_ADJ_VALIDITY_H with ORDER_LINE_NO=1,ADJUST_DTM='2016-08-08T16:54:16' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "where ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and ORDER_LINE_NO = 2" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_INF_BATCH_ADJ_VALIDITY_D with ORDER_LINE_NO=2,MOBILE_NO='0899999999' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "where ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and ORDER_LINE_NO = 3" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_INF_BATCH_ADJ_VALIDITY_T with ORDER_LINE_NO=3,COUNT_RECORD=4 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
