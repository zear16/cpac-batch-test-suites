...
echo -e "and R.REMARK = 'Invalid Format (column count mismatch, expected 2 but found 1)'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
echo "      ,REMARK='Invalid Format (column count mismatch, expected 2 but found 1)' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
