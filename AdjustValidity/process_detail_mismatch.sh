...
echo "found PM_PREPAID_LOAD_BATCH with BATCH_STATE=L,PROCESS_STATUS=FL,LOAD_TOTAL=1,LOAD_SUCCESS=0,LOAD_ERROR=1 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REMARK = 'Invalid Format (column count mismatch, expected 7 but found 1)'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
echo "      ,REMARK='Invalid Format (column count mismatch, expected 7 but found 1)' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
