...
echo -e "and T.MOBILE_NO = '0819017657' and T.ADJUST_VALIDITY = 40" >> ${SCRIPT}
echo -e "and T.TRANSPARENT_DATA1 = 'cPAC'" >> ${SCRIPT}
echo -e "and T.TRANSPARENT_DATA2 = 'ADJ_40_DAY'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_ADJUST_TRANSACTION with MOBILE_NO=0819017657,ADJUST_VALIDITY=40"
echo "      , TRANSPARENT_DATA1=cPAC, TRANSPARENT_DATA2=ADJ_40_DAY : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
tail -n 1 ${IN_PATH}/${ADJUST_FILE} >> ${OUT_PATH}/${ADJUST_NAME}.err
...
echo -e "use PMDB" > ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
echo -e "select REF.ADJUST_ID, REF.ACCOUNT_NO, OAD.SO_NBR, EAD.SO_NBR" >> ${SCRIPT}
echo -e "from PM_JOB_ORDER JO" >> ${SCRIPT}
echo -e "inner join" >> ${SCRIPT}
echo -e "(" >> ${SCRIPT}
echo -e "select PAT.ADJUST_ID, ${CHK_ORDER_ID} as ORDER_ID, PAT.SO_NBR, PAT.ADJUST_DATE, BP.VAT_RATE" >> ${SCRIPT}
echo -e ", PAT.ACCOUNT_NO" >> ${SCRIPT}
echo -e "from PM_FILE_ADJUST FD" >> ${SCRIPT}
echo -e "inner join PM_ADJUST_TRANSACTION PAT on (FD.FILE_ID = PAT.FILE_ID)" >> ${SCRIPT}
echo -e "left join CPDB..SFF_ACCOUNT BA on (PAT.ACCOUNT_NO = BA.ACCNT_NO)" >> ${SCRIPT}
echo -e "left join CPDB..SFF_BILLING_PROFILE BP on (BA.ROW_ID = BP.ACCNT_ID)" >> ${SCRIPT}
echo -e "where FD.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e ") REF on (JO.ORDER_ID = REF.ORDER_ID)" >> ${SCRIPT}
echo -e "left join PM_INF_ADJUST_BALANCE_D OAD on (OAD.ORDER_ID = JO.ORDER_ID" >> ${SCRIPT}
echo -e "and OAD.SO_NBR = REF.SO_NBR)" >> ${SCRIPT}
echo -e "left join PM_INF_ADJUST_BALANCE_ERROR_D EAD on (EAD.ORDER_ID = JO.ORDER_ID" >> ${SCRIPT}
echo -e "and EAD.SO_NBR = REF.SO_NBR)" >> ${SCRIPT}
echo -e "where JO.ORDER_ID = ${CHK_ORDER_ID}" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
#isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT}
...
# Cleanup check ORDER move to adjust_error.cleanup.sh
