...
echo -e "and R.FIELD_NAME = 'Request Date'" >> ${SCRIPT}
echo -e "and R.REMARK = 'Field Request Date invalid date format (20160808 16:35:4X)'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,FIELD_NAME='Request Date'"
echo "      ,REMARK='Field Request Date invalid date format (20160808 16:35:4X)' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
