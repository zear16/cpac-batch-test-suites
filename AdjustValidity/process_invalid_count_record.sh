...
echo -e "and R.FIELD_NAME = 'Count Record'" >> ${SCRIPT}
echo -e "and R.REMARK = 'Field Count Record invalid number format (found invalid character)'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,FIELD_NAME='Count Record'"
echo "      ,REMARK='Field Count Record invalid number format (found invalid character)' : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
