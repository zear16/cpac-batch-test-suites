...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_RECHARGE_REWARD W" >> ${SCRIPT}
echo -e "inner join PM_RECHARGE R on (W.RECHARGE_ID = R.RECHARGE_ID)" >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and W.REWARD_NAME = 'Reward0'" >> ${SCRIPT}
echo -e "and W.REWARD_AMT = 73.38" >> ${SCRIPT}
echo -e "\nprint '%1!', @count\n\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
if [ $RET -ne 0 ]; then
...
echo "found PM_RECHARGE_REWARD with REWARD_NAME=Reward0,REWARD_AMT=73.38 : ${COUNT}"
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and W.REWARD_NAME = 'Reward1'" >> ${SCRIPT}
echo -e "and W.REWARD_AMT = 55.03" >> ${SCRIPT}
echo -e "\nprint '%1!', @count\n\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
if [ $RET -ne 0 ]; then
...
echo "found PM_RECHARGE_REWARD with REWARD_NAME=Reward1,REWARD_AMT=55.03 : ${COUNT}"
