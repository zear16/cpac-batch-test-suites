...
echo -e "where B.ORDER_ID = ${ORDER_ID} and SEQ_NO = 1 and FIELD_NAME = 'PaymentType'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,FIELD_NAME=PaymentType : ${COUNT}"
