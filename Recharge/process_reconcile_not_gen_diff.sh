...
select @data_date = DATA_DATE_FR
from PM_JOB_ORDER 
where ORDER_ID = ${ORDER_ID}

print '@data_date=[%1!]', @data_date

create table #PM_RECHARGE_SERVICE (
  SERVICE_ROW_ID   unsigned bigint not null primary key,
  SERVICE_ID       unsigned bigint not null unique,
  DOCUMENT_TYPE_ID unsigned bigint not null,
  BANK_CODE        unsigned bigint null,
  GEN_RECEIPT_BOO  char(1)         not null,
  CASH_CARD_BOO    char(1)         not null
)

insert into #PM_RECHARGE_SERVICE (
SERVICE_ROW_ID, SERVICE_ID, DOCUMENT_TYPE_ID, BANK_CODE
, GEN_RECEIPT_BOO, CASH_CARD_BOO) 
select SERVICE_ROW_ID, SERVICE_ID, DOCUMENT_TYPE_ID, BANK_CODE
, GEN_RECEIPT_BOO, CASH_CARD_BOO 
from PM_RECHARGE_SERVICE 
where RECHARGE_BOO = 'Y' 
and EFFECTIVE_DATE <= getdate() 
and (EXPIRY_DATE = null or EXPIRY_DATE > getdate())

select RECHARGE_ID, RECHARGE_CHANNEL, RECEIPT_NO
from PM_RECHARGE
where RECHARGE_DATE = @data_date

select R.RECHARGE_ID, R.RECHARGE_CHANNEL as SERVICE_ID
, R.RECHARGE_PARTNER_ID as BANK_CODE, R.MOBILE_NO
, convert(char(8), R.RECHARGE_DATE, 112) as TRANSACTION_DATE
, R.RECHARGE_AMT as TOTAL_AMT
, R.CARD_BATCH_ID ||  R.CARD_SERIAL_NO  as BATCH_NO
, R.RECEIPT_NO, R.RECEIPT_DATE, R.RECEIPT_STATUS 
from PM_RECHARGE R 
inner join #PM_RECHARGE_SERVICE S on (R.RECHARGE_CHANNEL = S.SERVICE_ID) 
where R.RECHARGE_DATE = @data_date 
