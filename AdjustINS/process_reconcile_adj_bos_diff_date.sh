...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495"
echo "      ,TRANS_DTM=20160616,FACE_VALUE=938,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495"
echo "      ,TRANS_DTM=20160616,FACE_VALUE=null,VALIDITY=25 : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
