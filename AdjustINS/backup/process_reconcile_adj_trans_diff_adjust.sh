...
${EXECUTE_PATH}/reconcile -k -c ${CONFIG_NAME} --order-id ${ORDER_ID} --reconcile-code 007
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
#echo -e "and D.VALIDITY = 25" >> ${SCRIPT}
#echo -e "and RM.RECONCILE_CODE = '007'" >> ${SCRIPT}
#echo -e "\nprint '%1!', @count" >> ${SCRIPT}
#echo -e "\ngo" >> ${SCRIPT}
...
#echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495,TRANS_DTM=20160616,FACE_VALUE=938,VALIDITY=25 : ${COUNT}"
#if [ "$COUNT" -ne "1" ]; then
#  exit $RET_FAIL
#fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495,TRANS_DTM=20160616,FACE_VALUE=938,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
echo -e "and D.D_MOBILE_NO = '0811097495'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160616'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = null" >> ${SCRIPT}
echo -e "and D.VALIDITY = 25" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '007'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495,TRANS_DTM=20160616,FACE_VALUE=null,VALIDITY=25 : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_L_REF_2_RECORD = 4" >> ${SCRIPT}
echo -e "and R.REF_1_L_REF_2_AMT = 1876" >> ${SCRIPT}
echo -e "and R.REF_1_L_REF_2_VALIDITY = 50" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "      ,REF_1_RECORD=4,REF_1_AMT=1876,REF_1_VALIDITY=50,REF_2_RECORD=0,REF_2_AMT=0,REF_2_VALIDITY=0"
echo "      ,REF_1_L_REF_2_RECORD=4,REF_1_L_REF_2_AMT=1876,REF_1_L_REF_2_VALIDITY=50"
...
${EXECUTE_PATH}/reconcile -c ${CONFIG_NAME} --order-id ${ORDER_ID} --reconcile-code 008
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=008,D_MOBILE_NO=0811097495,TRANS_DTM=20160616,FACE_VALUE=938,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=008,LAST_BOO=Y,DIFF_BOO=N"
