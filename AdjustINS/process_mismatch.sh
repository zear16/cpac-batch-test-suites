...
echo -e "and R.SEQ_NO = 1 and R.REMARK = 'Invalid Format (column count mismatch, expected 41 but found 40)'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,REMARK=Invalid Format (column count mismatch, expected 41 but found 40) : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
