...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD1_VALUE='Y'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_ADD'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD2_VALUE='C'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_ADD'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD1_VALUE='N'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_DEDUCT'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD1_VALUE='${ADD_FLAG}'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_ADD'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD2_VALUE='${ADD_DOCUMENT}'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_ADD'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD1_VALUE='${DEDUCT_FLAG}'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_DEDUCT'
...
update PM_SYSTEM_ATTRIBUTE_DTL set FIELD1_VALUE='${DEDUCT_DOCUMENT}'
where ATTRIBUTE_CODE = 'PREPAID_PARAM'
and DB_VALUE = 'UFAIR_DOC_ADJUST_DEDUCT'
...
select A.RECEIPT_ID, A.NOTIFICATION_ID
from PM_FILE_ADJUST F
inner join PM_ADJUST_TRANSACTION A on (F.FILE_ID = A.FILE_ID)
where F.ORDER_ID = ${ORDER_ID}
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_ADJUST_TRANSACTION_CREDIT_NOTE with NOTIFICATION_ID!=NULL : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
inner join PM_ADJUST_TRANSACTION A on (F.FILE_ID = A.FILE_ID)
inner join PM_ADJUST_TRANSACTION_CREDIT_NOTE AC on (A.ADJUST_ID = AC.ADJUST_ID)
inner join PM_CREDIT_NOTE C on (AC.CN_ID = C.CN_ID and AC.CN_DATE = C.CN_DATE)
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_CREDIT_NOTE_DTL : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
from PM_JOB_ORDER
where REF_ORDER_ID = ${CHK_ORDER_ID}
and TEMPLATE_CODE = 'PE_SIEBEL_ACTIVITY_UFAIR'
...
select @count = count(*)
from PM_JOB_ORDER_PARAM
where ORDER_ID = ${ACT_ORDER_ID}
and PARAM_NAME = 'ORIG_ORDER_ID'
and PARAM_VALUE = '${ORDER_ID}'
