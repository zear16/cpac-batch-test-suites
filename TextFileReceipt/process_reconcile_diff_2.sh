...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_INF_BATCH_PP_RECEIPT"  >> ${SCRIPT}
...
echo "found PM_INF_BATCH_PP_RECEIPT : ${COUNT}"

if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi

${EXECUTE_PATH}/call_sproc -c ${CONFIG_NAME} PM_S_TX_LOAD_PM_BATCH_PP_RECEIPT_FROM_INF @order_id=${ORDER_ID}
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_BATCH_PP_RECEIPT R"  >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_BATCH_PP_RECEIPT : ${COUNT}"
...
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and MT.TABLE_NAME = 'PM_BATCH_PP_RECEIPT'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '013'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=013,TABLE_NAME=PM_BATCH_PP_RECEIPT : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=013,TABLE_NAME=PM_ADJUST_TRANSACTION : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0892500409'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160615'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = 66.68" >> ${SCRIPT}
echo -e "and D.RECEIPT_NO = 'W-R6-1609-1559-0000000001'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '013'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=013,D_MOBILE_NO=0892500409"
echo "                                           ,TRANS_DTM=20160615,FACE_VALUE=66.68,RECEIPT_NO=W-R6-1609-1559-0000000001 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0901000001'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160615'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = 100" >> ${SCRIPT}
echo -e "and D.RECEIPT_NO = 'XXX'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '013'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=013,D_MOBILE_NO=0901000001"
echo "                                           ,TRANS_DTM=20160615,FACE_VALUE=100,RECEIPT_NO=XXX : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_AMT = 66.68" >> ${SCRIPT}
...
echo -e "and R.REF_1_L_REF_2_AMT = 66.68" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=013,LAST_BOO=Y,DIFF_BOO=Y"
echo "      ,REF_1_RECORD=1,REF_1_AMT=66.68,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=1,REF_2_AMT=100,REF_2_VALIDITY=0"
echo "      ,REF_1_L_REF_2_RECORD=1,REF_1_L_REF_2_AMT=66.68,REF_1_L_REF_2_VALIDITY=0"
