...
RECONCILE_CODE=002,D_MOBILE_NO=0817037536,FACE_VALUE=101.00,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
and R.REF_1_M_REF_2_AMT = 101.00
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo -e "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=002,LAST_BOO=Y,DIFF_BOO=Y
   ,REF_1_RECORD=1,REF_1_AMT=3.00,REF_1_VALIDITY=0
   ,REF_2_RECORD=2,REF_2_AMT=104.00,REF_2_VALIDITY=0
   ,REF_1_L_REF_2_RECORD=0,REF_1_L_REF_2_AMT=0,REF_1_L_REF_2_VALIDITY=0
   ,REF_1_M_REF_2_RECORD=1,REF_1_M_REF_2_AMT=101,REF_1_M_REF_2_VALIDITY=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
