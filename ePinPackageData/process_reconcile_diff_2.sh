...
echo -e "\ngo" >> ${SCRIPT}
echo -e "set nocount on" >> ${SCRIPT}
echo -e "select RM.RECONCILE_CODE, MT.TABLE_NAME" >> ${SCRIPT}
...
echo -e "and RM.RECONCILE_CODE = '023'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo -e "and D.PACKAGE_CODE = 180001" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with D_MOBILE_NO=0901000009,TRANS_DTM=20160615,PACKAGE_CODE=180001,PACKAGE_FEE=1000 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with D_MOBILE_NO=0901000009,TRANS_DTM=20160615,PACKAGE_CODE=180001,PACKAGE_FEE=100 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
