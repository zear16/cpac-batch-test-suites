...
select @count = count(*)
from PM_BATCH_ROM_TOPUP R
inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)
where B.ORDER_ID = ${ORDER_ID}
and R.BRANCH_CODE = null
and R.AGENT_MOBILE_NO = '0901111111'
...
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_BATCH_ROM_TOPUP with BRANCH_CODE=NULL,AGENT_MOBILE_NO=0901111111  : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
