...
echo -e "select CN.CN_NO, CN.CATEGORY_CODE, CN.BOP_ID" >> ${SCRIPT}
echo -e "from PM_CREDIT_NOTE CN" >> ${SCRIPT}
echo -e "left join PM_PAYMENT_CATEGORY CT on (CN.CATEGORY_CODE = CT.CATEGORY_CODE)" >> ${SCRIPT}
echo -e "left join PM_BUSINESS_OF_PAYMENT BOP on (CN.BOP_ID = BOP.BOP_ID)" >> ${SCRIPT}
echo -e "where CN.CN_DATE = '20160616'" >> ${SCRIPT}
echo -e "and CT.CATEGORY_ABBR = 'PP'" >> ${SCRIPT}
echo -e "and BOP.BOP_CODE = 'I'" >> ${SCRIPT}
echo -e "order by CN.CN_DATE, CN.MOBILE_NO, CN.REFUND_TOTAL_AMT" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=020,TABLE_NAME=PM_CREDIT_NOTE : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=020,D_MOBILE_NO=0817053436,TRANS_DTM=20160616,FACE_VALUE=16,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_M_REF_2_AMT = 16" >> ${SCRIPT}
...
echo "      ,REF_1_M_REF_2_RECORD=1,REF_1_M_REF_2_AMT=16,REF_1_M_REF_2_VALIDITY=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
