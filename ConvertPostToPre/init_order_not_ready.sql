...
begin
  ...
  -- We intend to insert wrong number
  select @order_id = newid()

  insert into CPDB..SFF_ORDER
  (ROW_ID, ORDER_TYPE, STATUS_CD, STATUS_DT, COMPLETED_DT
  --, MODIFICATION_NUM, REQUEST_ID, WO_ID
  , SUBCONTRACTOR
  , CREATED_BY, CREATED, LAST_UPD_BY, LAST_UPD)
  values
  (@order_id, 'Convert Postpaid to Prepaid', 'Incomplete', @trans_date, @trans_date
  --, 0, 0, 0
  , ''
