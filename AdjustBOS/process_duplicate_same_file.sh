...
where ORDER_ID = ${ORDER_ID} and LOAD_TOTAL = 2 and LOAD_SUCCESS = 2 and LOAD_ERROR = 0
...
inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)
where B.ORDER_ID = ${ORDER_ID}
and R.SEQ_NO = 2 and R.FIELD_NAME = 'TRANSACTION_ID'
and R.REMARK = 'Duplicate TRANSACTION_ID'
...
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=2,
       FIELD_NAME=TRANSACTION_ID,REMARK=Duplicate TRANSACTION_ID : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
