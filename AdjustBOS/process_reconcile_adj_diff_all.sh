...
echo -e "where ORDER_ID = ${ORDER_ID} and LOAD_TOTAL = 4 and LOAD_SUCCESS = 4 and LOAD_ERROR = 0" >> ${SCRIPT}
...
echo "found PM_PREPAID_LOAD_BATCH with LOAD_TOTAL=4,LOAD_SUCCESS=4,LOAD_ERROR=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_INF_ADJUST_BOS I" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_INF_ADJUST_BOS : ${COUNT}"
if [ "$COUNT" -ne "4" ]; then
  exit $RET_FAIL
fi

# Expected OK
${EXECUTE_PATH}/call_sproc -c ${CONFIG_NAME} PM_S_TX_LOAD_PM_ADJUST_BOS_FROM_INF @order_id=${ORDER_ID}
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_ADJUST_BOS A"  >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_ADJUST_BOS : ${COUNT}"
if [ "$COUNT" -ne "4" ]; then
  exit $RET_FAIL
fi
...
${EXECUTE_PATH}/reconcile -k -c ${CONFIG_NAME} --order-id ${RECONCILE_ORDER_ID} --reconcile-code 007
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo -e "and RM.RECONCILE_CODE = '007'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=007,TABLE_NAME=PM_ADJUST_BOS : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${RECONCILE_ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0811097495'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160615'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = 938" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=007,D_MOBILE_NO=0811097495,TRANS_DTM=20160615,FACE_VALUE=938,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
and RM.RECONCILE_CODE = '007'
and R.DIFF_BOO = 'N'
and R.LAST_BOO = 'Y'
and R.REF_1_RECORD = 2
and R.REF_1_AMT = 1876
and R.REF_1_VALIDITY = 0
and R.REF_2_RECORD = 2
and R.REF_2_AMT = 1876
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=007,LAST_BOO=Y,DIFF_BOO=N"
echo "      ,REF_1_RECORD=2,REF_1_AMT=1876,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=2,REF_2_AMT=1876,REF_2_VALIDITY=0"
...
echo -e "and PROCESS_STATUS = 'PR'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH with BATCH_STATE=R,PROCESS_STATUS=PR : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi

${EXECUTE_PATH}/reconcile -c ${CONFIG_NAME} --order-id ${RECONCILE_ORDER_ID} --reconcile-code 008
RET=$?
if [ $RET -ne 0 ]; then
  exit $RET_FAIL
fi
...
echo -e "and RM.RECONCILE_CODE = '008'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=008,TABLE_NAME=PM_BATCH_DCB : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=008,TABLE_NAME=PM_RECEIPT : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=008,TABLE_NAME=PM_ADJUST_BOS : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=008,D_MOBILE_NO=0811097495,TRANS_DTM=20160615,FACE_VALUE=100,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=008,D_MOBILE_NO=0811097495,TRANS_DTM=20160615,FACE_VALUE=16,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=008,D_MOBILE_NO=0811097495,TRANS_DTM=20160615,FACE_VALUE=938,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "2" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_RECORD = 2" >> ${SCRIPT}
echo -e "and R.REF_1_AMT = 200" >> ${SCRIPT}
echo -e "and R.REF_1_VALIDITY = 0" >> ${SCRIPT}
echo -e "and R.REF_2_RECORD = 2" >> ${SCRIPT}
echo -e "and R.REF_2_AMT = 32" >> ${SCRIPT}
echo -e "and R.REF_2_VALIDITY = 0" >> ${SCRIPT}
echo -e "and R.REF_3_RECORD = 2" >> ${SCRIPT}
echo -e "and R.REF_3_AMT = 1876" >> ${SCRIPT}
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=008,LAST_BOO=Y,DIFF_BOO=Y"
echo "      ,REF_1_RECORD=2,REF_1_AMT=200,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=2,REF_2_AMT=32,REF_2_VALIDITY=0"
echo "      ,REF_3_RECORD=2,REF_3_AMT=1876,REF_3_VALIDITY=0"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
