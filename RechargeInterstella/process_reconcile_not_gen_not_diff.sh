...
echo "found PM_RECHARGE_GEN_RECEIPT : ${COUNT}"
...
echo -e "\ngo" >> ${SCRIPT}
echo -e "set nocount on" >> ${SCRIPT}
echo -e "select RP.SERVICE_ID, R.BANK_CODE, RD.MOBILE_NO, R.RECEIPT_DATE as TRANSACTION_DATE" >> ${SCRIPT}
echo -e ", RD.TOTAL_AMT, RDP.PREPAID_BATCH_NO || RDP.PREPAID_SERIAL_NO as BATCH_NO, R.RECEIPT_NO" >> ${SCRIPT}
echo -e ", R.RECEIPT_DATE, R.RECEIPT_STATUS" >> ${SCRIPT}
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0901033119'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160616'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = 7016" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=001,D_MOBILE_NO=0901033119,TRANS_DTM=20160616,FACE_VALUE=7016,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo "      ,REF_1_RECORD=1,REF_1_AMT=7016,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=1,REF_2_AMT=7016,REF_2_VALIDITY=0"
