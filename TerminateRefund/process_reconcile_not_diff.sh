...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=027 : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=027 : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=027,LAST_BOO=Y,DIFF_BOO=N"
