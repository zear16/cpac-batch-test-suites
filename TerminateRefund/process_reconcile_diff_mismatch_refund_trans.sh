...
echo -e "and MT.TABLE_NAME = 'PM_REFUND_TRANSACTION'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '027'" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=027,TABLE_NAME=PM_REFUND_TRANSACTION : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=027,D_MOBILE_NO=0817053436,TRANS_DTM=20160615,FACE_VALUE=18,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_AMT = 34" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=027,LAST_BOO=Y,DIFF_BOO=Y"
echo "      ,REF_1_RECORD=2,REF_1_AMT=34,REF_1_VALIDITY=0"
...
echo "      ,REF_1_M_REF_2_RECORD=0,REF_1_M_REF_2_AMT=0,REF_1_M_REF_2_VALIDITY=0"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
