...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=027,D_MOBILE_NO=0817053436,TRANS_DTM=20160615,FACE_VALUE=16,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "      ,REF_1_M_REF_2_RECORD=1,REF_1_M_REF_2_AMT=16,REF_1_M_REF_2_VALIDITY=0"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
