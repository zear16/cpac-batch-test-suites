...
#${EXECUTE_PATH}/reconcile -c ${CONFIG_NAME} --order-id ${ORDER_ID} --reconcile-code
...
echo -e "and D.PACKAGE_CODE = 170000" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with D_MOBILE_NO=0901000009,TRANS_DTM=20160615,PACKAGE_CODE=170000,PACKAGE_FEE=1000 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with D_MOBILE_NO=0901000009,TRANS_DTM=20160615,PACKAGE_CODE=170000,PACKAGE_FEE=100 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
