...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=026,TABLE_NAME=PM_ADJUST_TRANSACTION : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=026,S_MOBILE_NO=0817053436,D_MOBILE_NO=0811097495,TRANS_DTM=20160615,FACE_VALUE=18,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
