...
from PM_PAYMENT_CATEGORY where CATEGORY_ABBR = 'PP'
...
-- Test Case reconcile_not_gen_not_diff
exec _INSERT_RECEIPT @trans_date, @company_id, @company_code, @bop_id, @bop_code
, @doc_type_id, @doc_type, @channel_id, @channel_code, @category_code, @bank_code
, @period, 16, 'N', @location_code, @mobile_no
, 39, null, null, null
, @receipt_id out, @doc_no out

update PM_ADJUST_TRANSACTION set RECEIPT_ID = @receipt_id
, RECEIPT_DATE = @trans_date
where SO_NBR = 'Unit Test Adjust Wrong Number 1'
and ADJUST_DATE = @trans_date
and TRANS_NO = 0
and MOBILE_NO = @mobile_no

print '%1! row affected', @@rowcount
