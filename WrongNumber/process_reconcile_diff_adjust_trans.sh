...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=026,TABLE_NAME=PM_RECEIPT : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and D.D_MOBILE_NO = null" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=026,S_MOBILE_NO=0817053436,D_MOBILE_NO=null,TRANS_DTM=20160615,FACE_VALUE=16,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.S_MOBILE_NO = null" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=026,S_MOBILE_NO=null,D_MOBILE_NO='0811097495',TRANS_DTM=20160615,FACE_VALUE=16,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_3_AMT = 16" >> ${SCRIPT}
...
echo "      ,REF_3_RECORD=1,REF_3_AMT=16,REF_3_VALIDITY=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
