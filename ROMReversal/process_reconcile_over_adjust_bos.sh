...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF_TABLE with RECONCILE_CODE=014,TABLE_NAME=PM_ADJUST_BOS : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=014,D_MOBILE_NO=0854000673,FACE_VALUE=16,TRANS_DTM=20160615 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_3_AMT = 19.07" >> ${SCRIPT}
...
echo "      ,REF_3_RECORD=2,REF_3_AMT=19.07,REF_3_VALIDITY=0"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
