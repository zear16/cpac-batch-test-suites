...
echo -e "\ngo" >> ${SCRIPT}
echo -e "set nocount on" >> ${SCRIPT}
echo -e "select R.SEQ_NO, R.FIELD_NAME, R.REMARK" >> ${SCRIPT}
echo -e "from PM_PREPAID_LOAD_BATCH_REJECT R"  >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
#isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} -Jutf8
...
echo -e "and R.FIELD_NAME = 'COMPANY_ACCOUNT_NO'" >> ${SCRIPT}
echo -e "and R.REMARK = 'Invalid COMPANY_ACCOUNT_NO'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=1,FIELD_NAME=COMPANY_ACCOUNT,REMARK=Invalid COMPANY_ACCOUNT_NO : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
