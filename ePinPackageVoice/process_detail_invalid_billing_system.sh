...
${EXECUTE_PATH}/call_sproc -c ${CONFIG_NAME} PM_S_TX_LOAD_PM_BATCH_PACKAGE_TOPUP_VOICE_FROM_INF @order_id=${ORDER_ID}
RET=$?
if [ $RET -ne 1 ]; then
  exit $RET_FAIL
fi
...
echo -e "and TOTAL_AMT = 16" >> ${SCRIPT}
echo -e "and SUCCESS_AMT = 0" >> ${SCRIPT}
echo -e "and ERROR_AMT = 16" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo -e "found PM_PREPAID_LOAD_BATCH with BATCH_STATE=C,PROCESS_STATUS=FL"
echo -e "      ,TOTAL_AMT=16,SUCCESS_AMT=0,ERROR_AMT=16 : ${COUNT}"
