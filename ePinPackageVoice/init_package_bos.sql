...
(COMPANY_ID, DOCUMENT_TYPE_ID, TEMPLATE_ID, RECEIPT_NO
, RECEIPT_DATE, MODE, RECEIPT_LOCATION_CODE
...
values
(@company_id, @document_type_id, @template_id, 'Unit Test ePin Package Voice'
, dateadd(dd, -1, @run_date), 'A', 1
...
begin
  ...
  (PACKAGE_CODE, PACKAGE_NAME, PACKAGE_TYPE, PACKAGE_FEE, ACTIVE_BOO, EFFECTIVE_DATE
  , BANK_CODE, GEN_RECEIPT_BOO
  , CREATED_BY, CREATED, LAST_UPD_BY, LAST_UPD)
  values
  (160000, @package_name, 'V', 100, 'Y', @run_date
  , @bank_code, 'N'
  , 'unit', getdate(), 'unit', getdate())

  select @package_code = 160000
...
, @version_id, "Unit Test ePin Package Voice", "Unit Test ePin Package Voice", "/opt/ais/cpac/batchprepaid/package", "", @job_chain, "W")
