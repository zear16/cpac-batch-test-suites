...
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
echo -e "where B.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and D.D_MOBILE_NO = '0892500816'" >> ${SCRIPT}
echo -e "and D.TRANS_DTM = '20160615'" >> ${SCRIPT}
echo -e "and D.FACE_VALUE = -39.94" >> ${SCRIPT}
echo -e "and D.CN_NO = 'N6-1609-1559-0000000000'" >> ${SCRIPT}
echo -e "and RM.RECONCILE_CODE = '012'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=012,D_MOBILE_NO=0892500816"
echo "                                           ,TRANS_DTM=20160615,FACE_VALUE=-39.94,CN_NO=N6-1609-1559-0000000000 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "\ngo" >> ${SCRIPT}
echo -e "set nocount on" >> ${SCRIPT}
echo -e "select D.D_MOBILE_NO, convert(char(8),D.TRANS_DTM,112), D.FACE_VALUE, D.CN_NO " >> ${SCRIPT}
echo -e "from PM_PREPAID_BATCH_RECONCILE_DIFF D" >> ${SCRIPT}
echo -e "inner join PM_PREPAID_BATCH_RECONCILE R on (D.BATCH_RCC_ID = R.BATCH_RCC_ID)" >> ${SCRIPT}
echo -e "inner join PM_RECONCILE_MAPPING RM on (R.RECONCILE_ID = RM.RECONCILE_ID)" >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH B on (R.BATCH_ID = B.BATCH_ID)" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=012,D_MOBILE_NO=0901000001"
echo "                                           ,TRANS_DTM=20160615,FACE_VALUE=-100,CN_NO=${CN_NO} : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "and R.REF_1_L_REF_2_AMT = -39.94" >> ${SCRIPT}
...
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=012,LAST_BOO=Y,DIFF_BOO=Y"
echo "      ,REF_1_RECORD=1,REF_1_AMT=-39.94,REF_1_VALIDITY=0"
echo "      ,REF_2_RECORD=1,REF_2_AMT=-100,REF_2_VALIDITY=0"
echo "      ,REF_1_L_REF_2_RECORD=1,REF_1_L_REF_2_AMT=-39.94,REF_1_L_REF_2_VALIDITY=0"
