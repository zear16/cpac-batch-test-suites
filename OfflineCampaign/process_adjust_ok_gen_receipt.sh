...
echo -e "\nset nocount on" >> ${SCRIPT}
echo -e "declare @count int" >> ${SCRIPT}
echo -e "select @count = count(*) from PM_CAMPAIGN_PROJECT where PROJECT_CODE = 'Cash Back'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count" >> ${SCRIPT}
echo -e "\ngo" >> ${SCRIPT}
...
if [ "$COUNT" = "0" ]; then
  ...
  echo -e "\ngo" >> ${SCRIPT}
  echo -e "\nset nocount on" >> ${SCRIPT}
  echo -e "insert into PM_CAMPAIGN_PROJECT" >> ${SCRIPT}
  echo -e "(PROJECT_CODE, BANK_CODE, SERVICE_ID, ACTIVE_BOO" >> ${SCRIPT}
  echo -e ", CREATED_BY, CREATED, LAST_UPD_BY, LAST_UPD)" >> ${SCRIPT}
  echo -e "values" >> ${SCRIPT}
  echo -e "('Cash Back', 1902, 900, 'Y'" >> ${SCRIPT}
...
select PAT.ADJUST_ID, PAT.MOBILE_NO, PAT.CREATED, PAT.BOS_AMOUNT_ADJUST
, convert(varchar(20),CP.BANK_CODE), PAT.COMPANY_ID, PAT.ADJUST_DTM
, RS.SERVICE_ID, PAT.GEN_DOC_TYPE, CR.CATEGORY_CODE
, case when PAT.GEN_DOC_TYPE = 'RT' then CR.RC_SUB_BOP_ID
       when PAT.GEN_DOC_TYPE = 'CN' then CR.CN_SUB_BOP_ID
  else null end as SUB_BOP_ID
, BC.PROJECT_CODE
from PM_ADJUST_TRANSACTION PAT
inner join PM_FILE_ADJUST FA on (PAT.FILE_ID = FA.FILE_ID)
inner join PM_BATCH_CAMPAIGN BC on (PAT.REF_ID = BC.CP_ID)
inner join PM_CAMPAIGN_PROJECT CP on (BC.PROJECT_CODE = CP.PROJECT_CODE)
inner join PM_RECHARGE_SERVICE RS on (CP.SERVICE_ID = RS.SERVICE_ID and CP.BANK_CODE = RS.BANK_CODE)
inner join PM_CFG_RECHARGE_SERVICE CR on (RS.SERVICE_ROW_ID = CR.SERVICE_ROW_ID)
where FA.ORDER_ID = ${ORDER_ID}
and PAT.BOS_AMOUNT_ADJUST is not null
and PAT.RECEIPT_ID is null
and PAT.ADJUST_STATUS = 'SC'
and PAT.TRANSPARENT_DATA2 = 'CAMPAIGN'
