...
if [ "${COUNT}" -ne "0" ]; then
  ...
  echo -e "\ngo" >> ${SCRIPT}
  echo -e "\nset nocount on" >> ${SCRIPT}
  echo -e "delete from PM_CAMPAIGN_PROJECT where PROJECT_CODE = 'Cash Back'" >> ${SCRIPT}
  echo -e "\ngo" >> ${SCRIPT}
...
select ADC.CN_ID as AD_CN_ID, ADC.CN_DATE as AD_CN_DATE, AD.RECEIPT_ID
, AD.MOBILE_NO as AD_MOBILE_NO, AD.BILLING_SYSTEM, AD.ADJUST_AMT, AD.ADJUST_STATUS
, AD.FILE_ID, CN.CN_NO, PMDB..PM_F_FORMAT_DATE(CN.CN_DATE, 'YYYY-MM-DD') as CN_DATE
, CR.COMPANY_ABBR, CN.MODE, CN.CN_LOCATION_CODE
, convert(varchar(5), CT.CATEGORY_ABBR) as CATEGORY_ABBR
, convert(varchar(2),BOP.BOP_CODE) as BOP_CODE, CN.CN_STATUS, CN.RECEIPT_SENDING
, CN.CN_TYPE, CN.USER_ID, CN.ACC_REASON_ID, CN.REFUND_TOTAL_AMT, CN.MOBILE_NO 
