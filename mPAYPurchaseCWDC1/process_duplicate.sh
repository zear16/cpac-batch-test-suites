...
echo -e "where ORDER_ID = ${ORDER_ID} and LOAD_TOTAL = 2 and LOAD_SUCCESS = 2 and LOAD_ERROR = 0" >> ${SCRIPT}
...
echo "found PM_PREPAID_LOAD_BATCH with LOAD_TOTAL=2,LOAD_SUCCESS=2,LOAD_ERROR=0 : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
...
echo -e "select @count = count(*)" >> ${SCRIPT}
echo -e "from PM_PREPAID_LOAD_BATCH_REJECT R" >> ${SCRIPT}
echo -e "inner join PM_PREPAID_LOAD_BATCH L on (R.BATCH_ID = L.BATCH_ID)" >> ${SCRIPT}
echo -e "where L.ORDER_ID = ${ORDER_ID}" >> ${SCRIPT}
echo -e "and L.BATCH_STATE = 'C' and L.PROCESS_STATUS = 'FL'" >> ${SCRIPT}
echo -e "and R.SEQ_NO = 2 and R.FIELD_NAME='TRANSACTION_ID'" >> ${SCRIPT}
echo -e "and R.REMARK='Duplicate TRANSACTION_ID'" >> ${SCRIPT}
echo -e "\nprint '%1!', @count\n\ngo" >> ${SCRIPT}
isql -U${USER} -P${PASS} -S${SERVER} -i ${SCRIPT} > ${WORKING_PATH}/count
RET=$?
if [ $RET -ne 0 ]; then
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_LOAD_BATCH_REJECT with SEQ_NO=2,FIELD_NAME=TRANSACTION_ID,REMARK=Duplictate TRANSACTION_ID : ${COUNT}"
if [ "$COUNT" -ne "1" ]; then
  exit $RET_FAIL
fi
