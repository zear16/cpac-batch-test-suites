...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE_DIFF with RECONCILE_CODE=002,D_MOBILE_NO=0817037536"
echo "      ,TRANS_DTM=20160805,FACE_VALUE=100,VALIDITY=null : ${COUNT}"
if [ "$COUNT" -ne "0" ]; then
  exit $RET_FAIL
fi
...
COUNT=$(cat ${WORKING_PATH}/count)
echo "found PM_PREPAID_BATCH_RECONCILE with RECONCILE_CODE=002,LAST_BOO=Y,DIFF_BOO=N"
